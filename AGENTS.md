...
design project's narrative, an Introduction edit) do not trigger
re-export.
...
4. Did this change trigger a CV re-export? If yes, is the new PDF in
   `assets/`?
...
# Full build of both (data → Quarto → _site)
./build.sh
```
...
Tier 3 Phase B will render the CV PDFs directly from `_data/*.yml` via
a Typst pipeline (`cv/cv.typ` + `scripts/build_cv.py`). Once that lands
the "CV re-export" section above collapses to "the build takes care of
it" and the manual re-export triggers disappear.
